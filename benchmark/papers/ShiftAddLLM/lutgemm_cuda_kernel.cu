...
void vecquant3matmul_cuda(
  torch::Tensor vec, // [a]
  torch::Tensor mat, // [(infeatures // 32 * 3, outfeatures]
  torch::Tensor mul, // [outfeatures]
  torch::Tensor scales, // [outfeatures, 1]
  torch::Tensor zeros // [outfeatures, 1]
) {
...
__global__ void VecQuant3MatMulKernel(
  ...
  blockvec[threadIdx.x] = vec[(row / BLOCKHEIGHT) * BLOCKWIDTH + threadIdx.x]; // out of bounds
  __syncthreads();

  scalar_t scale = scales[col]; // out of bounds
  scalar_t zero = zeros[col]; // out of bounds

  scalar_t res = 0;
  int i = width * row + col; // integer overflow
  int k = 0;
  ...
  while (k < BLOCKWIDTH) {
    tmp1 = as_unsigned(mat[i]); // out of bounds
