...
# # [新增] 修复 torchmock 导致的 torch._dynamo 崩溃
# # torchmock 替换了 torch.cuda.Event，导致 dynamo 的 issubclass 检查失败。
# # 手动构造一个假的 torch._dynamo 模块，并填充必要的 dummy 函数，
# # 防止 torch.distributed 等库 import 失败。
# # =============================================================================
import sys, types

# 1. 构造假的 torch._dynamo 模块对象
fake_dynamo = types.ModuleType("torch._dynamo")

# 2. 定义通用的占位函数 (Identity Function)
# assume_constant_result 的作用是告诉编译器某结果是常量，mock 时直接返回原值即可
def _identity(obj, *args, **kwargs):
    return obj

# 3. 定义通用的装饰器生成器 (Mock optimize/disable)
...
def testAqlmManual(override_configs=None, out_dir=None, data_dir=None, op_name=None):
    ...
    # --- 2. 在 NvccPatch 保护下导入 ---
    # 利用全局 patch_torch_cpp_extension 机制，这里不需要额外 mock load
...
def testMCM(override_configs=None, out_dir=None, data_dir=None, op_name=None):
    ...
    print(f"开始替换 Linear 层 (MCM, bits={mcm_bits})...")
    with NvccPatch():
...
def testAnyPrecision(override_configs=None, out_dir=None, data_dir=None, op_name=None):
    ...
    repo_root = '/home/mvh6224/CUDA-BOSolver/pyanalyzer/models_rs/any-precision-llm'
    if repo_root not in sys.path:
        sys.path.insert(0, repo_root)
    ...
    print(f"开始替换 Linear 层 (AnyPrecision, bits={target_bits})...")
    with NvccPatch():
