...
class LoadedCppExtensionMock:
    ...
    def __getattr__(self, item):
        def _mocked_method(*args, **kwargs):
            ...
            if item.startswith("code") and ("matmat" in item or "matvec" in item):
                try:                    
                    input_tensor = args[0]
            ...
            if item == "dequant_kbit":
                if len(args) > 0 and torch.is_tensor(args[0]):
                    qweight = args[0]
                    if qweight.dim() == 3:
                        out_features = qweight.shape[1]
                        in_features = qweight.shape[2] * 32
                        return torch.zeros((out_features, in_features), dtype=torch.float16, device=qweight.device)
                    else:
