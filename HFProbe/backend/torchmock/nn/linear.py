...
class MockLinearFunction(torch.autograd.Function):
    @staticmethod
    def forward(ctx, x, weight, bias=None, **kwargs):
    ...
    def backward(ctx, grad_output):
        ...
        if ctx.needs_input_grad[0]:
            grad_input = torch.zeros(x_shape, device=grad_output.device, dtype=grad_output.dtype)
        if ctx.needs_input_grad[1]:
            grad_weight = torch.zeros(weight_shape, device=grad_output.device, dtype=grad_output.dtype)
        if bias_shape is not None and ctx.needs_input_grad[2]:
            grad_bias = torch.zeros(bias_shape, device=grad_output.device, dtype=grad_output.dtype)
...
def pure_linear_mock(x, weight, bias=None):
