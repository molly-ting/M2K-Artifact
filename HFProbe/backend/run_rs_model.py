...
def testShiftAdd(override_configs=None, out_dir=None, op_name=None):
    ...
    # local_dir = snapshot_download(modelId, ignore_patterns=["*.bin", "*.safetensors"])
    # # sys.path.append(local_dir)
    # print(f"Model {modelId} downloaded to {local_dir}")
    # createEmptyModelBin(modelId, local_dir)
    # copy_config_to_modules_if_needed(local_dir)
    # tokenizer = AutoTokenizer.from_pretrained(local_dir, trust_remote_code=True)
    # config = AutoConfig.from_pretrained(local_dir, trust_remote_code=True)
    # model = AutoModelForCausalLM.from_pretrained(local_dir, config=config, trust_remote_code=True) 

    torch.set_default_dtype(torch.float)
    parent_dir = "/home/mvh6224/CUDA-BOSolver/pyanalyzer/models_rs/ShiftAddLLM"
    ...
    # single_prompt = "word" * 9
    # single_prompt = single_prompt.strip() 
    inputs = tokenizer(["word"], return_tensors="pt", add_special_tokens=False)
    tokens = inputs["input_ids"]
    # print(tokens.shape)
    batch_size_real = len(tokens)
    seq_len_real = len(tokens[0])
    print(batch_size_real, seq_len_real)
    
    data = []
    out = model.generate(**inputs)
    data.append({"batch_size": 1, "seq_len": 1, "calls": tensor_calls.copy()})
    tensor_calls.clear()

    if not out_dir:
        out_dir = "/home/mvh6224/CUDA-BOSolver/pyanalyzer/rsdata"
...
def testKVQuant():
    ...
    output_dir = "/home/mvh6224/CUDA-BOSolver/pyanalyzer/rsout"
    ...
    with NvccPatch():
        ...
        model = AutoModelForCausalLM.from_pretrained(local_dir, config=config, torch_dtype=torch.half, use_flash_attention_2=True, device_map="cuda")
        # model.seqlen = seqlen
        model.model.set_devices()
        
        for seq_len in SEQ_LENS_CONFIGS: # only support batch_size=1
            print(f"\n--- 正在运行: batch_size=1, seq_len={seq_len} ---")
            # 1. 准备批量输入
            prompt_text = ("hi " * seq_len).strip()
            prompts = [prompt_text]
        
            # 2. 获取真实的token长度
            real_seq_len = len(tokenizer(prompt_text, return_tensors="pt")["input_ids"][0])
            print(f"目标 seq_len={seq_len}, 实际 token 长度={real_seq_len}")
            ...
            for call in tensor_calls:
                ...
                total_calls_map.setdefault(func_name, {}).setdefault(call_stack, {}).setdefault(run_key, []).append(argCons)
            data.append({"batch_size": 1, "seq_len": real_seq_len, "calls": tensor_calls.copy()})

    print("\n--- 所有推理运行完毕。开始符号化分析... ---")
    
    with open("/home/mvh6224/CUDA-BOSolver/pyanalyzer/rsdata/"+filename, "w") as wf:
        json.dump(data, wf)
    ...
    try:
        ...
        print(f"\n[错误] 生成符号化日志失败: {e}")
        # 如果失败，保存原始的 map 数据以供调试
        fallback_path = os.path.join(output_dir, f"{modelId.replace('/', '_')}.raw_map.json")
        # 为了能将 tuple 作为 key 序列化，进行转换
        serializable_map = {k: {str(k2): v2 for k2, v2 in v.items()} for k, v in total_calls_map.items()}
        with open(fallback_path, "w", encoding="utf-8") as f:
            json.dump(serializable_map, f, indent=2, ensure_ascii=False)
...
def testQuaRot(override_configs=None, out_dir=None, op_name=None):
    ...
    parent_dir = "/home/mvh6224/CUDA-BOSolver/pyanalyzer/models_rs/QuaRot"
    sys.path.append(parent_dir)
    ...
    # from quant_utils import add_actquant, find_qlayers, ActQuantWrapper
    # add_actquant(model)
    ...
    #     if 'v_proj' in name: #Set the v_proj precision
    #         qlayers[name].out_quantizer.configure(bits=4,
    #                                         groupsize=-1,
    #                                         sym=True,
    #                                         clip_ratio=1.0)
    ...
    # from model_utils import get_rope_function_name, get_layers
    # rope_function_name = get_rope_function_name(model)
    # layers = get_layers(model)
    # k_quant_config = {'k_bits':4, "k_groupsize": -1,
    #                                 "k_sym": True, "k_clip_ratio": 1.0}
    
    # from rotation_utils import add_qk_rotation_wrapper_after_function_call_in_forward
    # for layer in layers:
    ...
    run_once(model, tokenizer, config, prompts, new_tokens=NEW_TOKENS)
    # out = model.generate(**inputs)
    data.append({"batch_size": 3, "seq_len": 9, "calls": tensor_calls.copy()})
    tensor_calls.clear()
