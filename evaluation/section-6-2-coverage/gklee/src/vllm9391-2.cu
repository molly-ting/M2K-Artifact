...
int main() {
    ...
    cudaMalloc(&d_azp, sizeof(float));

    float scale = 10.0f;
    float azp = 0.0f;
    cudaMemset(d_input, 0, in_bytes);   // Initialize to zero instead of copying
    cudaMemset(d_output, 0, out_bytes);
    cudaMemcpy(d_scale, &scale, sizeof(float), cudaMemcpyHostToDevice);
    cudaMemcpy(d_azp, &azp, sizeof(float), cudaMemcpyHostToDevice);
