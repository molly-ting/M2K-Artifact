...
int main() {
    ...
    CUDA_CHECK(hipMalloc(&d_block_table, num_blocks * sizeof(int)));
    CUDA_CHECK(hipMalloc(&d_kv_cache, kv_cache_size * sizeof(float)));

    CUDA_CHECK(hipMemcpy(
        d_block_table, h_block_table,
        num_blocks * sizeof(int), hipMemcpyHostToDevice));
    CUDA_CHECK(hipMemset(d_kv_cache, 0, kv_cache_size * sizeof(float)));
    ...
    CUDA_CHECK(hipFree(d_block_table));
    CUDA_CHECK(hipFree(d_kv_cache));
