    ...
    char* __restrict__ out)
{
    ...
    for (int i = tid; i < hidden_size; i += blockDim.x) {
        ...
        // - Index wraps around to negative or incorrect value
        // - Causes out-of-bounds memory access
        // - Wrong elements get quantized
        // - Memory corruption
        int index = token_idx * hidden_size + i;
        
        float const val = input[index];
        int const quant_val = int32_to_int8(float_to_int32_rn(val / scale) + azp);
        out[index] = quant_val;
    }
}
...
int main() {
    ...
    ESBMC_verify_kernel(static_scaled_int8_azp_quant_kernel, grid, block,
        h_input,
        h_output);
