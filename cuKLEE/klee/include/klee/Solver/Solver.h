...
namespace klee {
  ...
  public:
    ...
    Query(const ConstraintSet& _constraints, ref<Expr> _expr, std::map<std::string, unsigned> _intArrNames, std::vector<ref<Expr>> _noCheckExprs)
      : constraints(_constraints), expr(_expr), intArrNames(_intArrNames), noCheckExprs(_noCheckExprs)  {
    }

    /// withExpr - Return a copy of the query with the given expression.
    Query withExpr(ref<Expr> _expr) const {
      return Query(constraints, _expr, intArrNames, noCheckExprs);
    }

    /// withFalse - Return a copy of the query with a false expression.
    Query withFalse() const {
      return Query(constraints, ConstantExpr::alloc(0, Expr::Bool), intArrNames, noCheckExprs);
    }
  ...
  public:
    ...
    bool evaluate(const Query&, Validity &result, bool forceSolve=false);
