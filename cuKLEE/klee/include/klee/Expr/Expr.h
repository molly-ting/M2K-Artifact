...
public:
  ...
  // static ref<Expr> createIntTempRead(const Array *array, Expr::Width w);
  ...
  static unsigned countBoolLogic(ref<Expr> e);
  static bool containRem(ref<Expr> e);
  static ref<Expr> andExprUsingArithmetic(ref<Expr> a, ref<Expr> b);
  static ref<Expr> orExprUsingArithmetic(ref<Expr> a, ref<Expr> b);
  static ref<Expr> rightShiftUsingArithmetic(ref<Expr> a, ref<Expr> b);
  static ref<Expr> leftShiftUsingArithmetic(ref<Expr> a, ref<Expr> b);
  static bool isRelatedWithThread(ref<Expr> e);
  static bool isRelatedWithBlock(ref<Expr> e);
  static bool isRelatedWithLoopIndex(ref<Expr> e, ref<Expr> loopIndex);
  static ref<ConstantExpr> getBaseAddress(ref<Expr> e);
  // static bool containsExpr(const ref<Expr> &A, const ref<Expr> &B);
  static void findAllConcatExpr(const ref<Expr> &e,
...
protected:
  BinaryExpr(const ref<Expr> &l, const ref<Expr> &r, bool _isSigned=false) : left(l), right(r), isSigned(_isSigned)  {}
...
private:
  ...
  Array(const std::string &_name, uint64_t _size, bool _isIntVar=false,
...
public:
  ...
  static ref<SelectExpr> findSelect(ref<Expr> e);
...
public:
  ...
  static bool checkIsReadInt(ref<Expr> e, const Array **array);
  static ref<ConstantExpr> getLastConstantInConcat(ref<Expr> expr, const Array **array);
  static ref<Expr> getArrayIndex(ref<Expr> e, const Array **array);
...
#define ARITHMETIC_EXPR_CLASS(_class_kind)                                     \
  ...
  public:                                                                      \
    ...
    static const unsigned numKids = 2;                                         \
                                                                               \
  public:                                                                      \
    _class_kind##Expr(const ref<Expr> &l, const ref<Expr> &r, bool _isSigned)                  \
        : BinaryExpr(l, r, _isSigned) {}                                                  \
    static ref<Expr> alloc(const ref<Expr> &l, const ref<Expr> &r, bool _isSigned=false) {           \
      ref<Expr> res(new _class_kind##Expr(l, r, _isSigned));                              \
      res->computeHash();                                                      \
      return res;                                                              \
    }                                                                          \
    static ref<Expr> create(const ref<Expr> &l, const ref<Expr> &r, bool _isSigned=false);           \
    Width getWidth() const { return left->getWidth(); }                        \
    Kind getKind() const { return _class_kind; }                               \
    virtual ref<Expr> rebuild(ref<Expr> kids[]) const {                        \
      return create(kids[0], kids[1], isSigned);                                         \
    }                                                                          \
                                                                               \
    static bool classof(const Expr *E) {                                       \
      return E->getKind() == Expr::_class_kind;                                \
...
private:
  ...
  ConstantExpr(const llvm::APInt &v, bool _isSigned, bool _isFloat) : value(v), isSigned(_isSigned), isFloat(_isFloat) {}
...
public:
  ...
  static float getFloatFromConstantExpr(ref<Expr> expr);
  static double getDoubleFromConstantExpr(ref<Expr> expr);

  static ref<ConstantExpr> alloc(const llvm::APInt &v, bool _isSigned=false, bool _isFloat=false) {
    ref<ConstantExpr> r(new ConstantExpr(v, _isSigned, _isFloat));
    r->computeHash();
    return r;
  }

  static ref<ConstantExpr> alloc(const llvm::APFloat &f, bool _isSigned=false, bool _isFloat=false) {
    return alloc(f.bitcastToAPInt(), _isSigned, _isFloat);
  }

  static ref<ConstantExpr> alloc(uint64_t v, Width w, bool _isSigned=false, bool _isFloat=false) {
    return alloc(llvm::APInt(w, v), _isSigned, _isFloat);
  }

  static ref<ConstantExpr> create(uint64_t v, Width w, bool _isSigned=false, bool _isFloat=false) {
#ifndef NDEBUG
    if (w <= 64)
      assert(v == bits64::truncateToNBits(v, w) && "invalid constant");
#endif
    return alloc(v, w, _isSigned, _isFloat);
  }
