...
ref<Expr> ConstraintManager::replaceExpr(ref<Expr> expr, ref<Expr> src, ref<Expr> dst) {
  return ExprReplaceVisitor(src, dst).visit(expr);
}

ref<Expr> ConstraintManager::replaceReadExpr(ref<Expr> expr, const std::map< ref<Expr>, ref<Expr> > &replacements) {
  return ExprReplaceVisitor2(replacements).visit(expr);
}
...
ref<Expr> ConstraintSet::get_back() const { return constraints.back(); }
