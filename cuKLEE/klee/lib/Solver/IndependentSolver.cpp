...
bool IndependentSolver::computeValidity(const Query& query,
  ...
  return solver->impl->computeValidity(Query(tmp, query.expr, query.intArrNames, query.noCheckExprs), 
                                       result);
}
...
bool IndependentSolver::computeTruth(const Query& query, bool &isValid) {
  ...
  IndependentElementSet eltsClosure = 
    getIndependentConstraints(query, required);
  ConstraintSet tmp(required);
  return solver->impl->computeTruth(Query(tmp, query.expr, query.intArrNames, query.noCheckExprs), 
                                    isValid);
}
...
bool IndependentSolver::computeValue(const Query& query, ref<Expr> &result) {
  ...
  IndependentElementSet eltsClosure = 
    getIndependentConstraints(query, required);
  ConstraintSet tmp(required);
  return solver->impl->computeValue(Query(tmp, query.expr, query.intArrNames, query.noCheckExprs), result);
}
...
bool IndependentSolver::computeInitialValues(const Query& query,
  ...
  for (std::list<IndependentElementSet>::iterator it = factors->begin();
    ...
    if (!solver->impl->computeInitialValues(Query(tmp, ConstantExpr::alloc(0, Expr::Bool), query.intArrNames, query.noCheckExprs),
