...
public:
  ...
  bool evaluate(const ConstraintSet &, ref<Expr>, Solver::Validity &result,
                SolverQueryMetaData &metaData, std::map<std::string, unsigned> intArrNames, std::vector<ref<Expr>> noCheckExprs, bool forceSolve=false);
  ...
  bool getValue(const ConstraintSet &, ref<Expr> expr,
                ref<ConstantExpr> &result, SolverQueryMetaData &metaData, std::map<std::string, unsigned> intArrNames={}, std::vector<ref<Expr>> noCheckExprs={});
