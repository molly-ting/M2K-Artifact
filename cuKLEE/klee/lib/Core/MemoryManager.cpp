...
llvm::cl::opt<unsigned> DeterministicAllocationSharedSize(
    "kdalloc-shared-size",
    llvm::cl::desc("Reserved memory for globals in MB (default=500)"),
    llvm::cl::init(500), llvm::cl::cat(MemoryCat));
...
llvm::cl::opt<std::uintptr_t> DeterministicAllocationSharedStartAddress(
    "kdalloc-shared-start-address",
    llvm::cl::desc(
        "Start address for shared segment (has to be page aligned)"),
    llvm::cl::cat(MemoryCat));
...
MemoryManager::MemoryManager(ArrayCache *_arrayCache)
  ...
  if (DeterministicAllocation) {
    ...
    requestedSegments.emplace_back(
        "shared",
        DeterministicAllocationSharedStartAddress
            ? DeterministicAllocationSharedStartAddress.getValue()
            : 0x10000,
        static_cast<std::size_t>(
            DeterministicAllocationSharedSize.getValue()) *
            1024 * 1024,
        sharedFactory, &sharedAllocator);
    requestedSegments.emplace_back(
        "globals",
...
MemoryObject* MemoryManager::allocateShared(
    uint64_t size,
    ...
    const llvm::Value *allocSite) 
{
    ...
    uint64_t address = 0;
    if (DeterministicAllocation) {
      void *allocAddress = sharedAllocator.allocate(
            std::max(size, static_cast<std::uint64_t>(4)));
    ...
    ++stats::allocations;
    MemoryObject *res = new MemoryObject(address, size, 4, false, true, false, allocSite, this);
    objects.insert(res);
    return res;
}
