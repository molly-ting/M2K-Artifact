...
#include "SymArrayMemory.h"
#include "TensorSizesMemory.h"
...
public:
  ...
  bool inMergeRegion = false; // true after openMerge, false after closeMerge 
  std::map<llvm::BasicBlock *, KLoopInfo *> loopInfoMap;
  std::map<uint64_t, std::string> symNames; // when the address of symbolic array is not the base address of the memory object, store the mapping
  std::map<uint64_t, uint64_t> symAddressMap; // when the address of symbolic array is not the base address of the memory object, store the mapping
  std::map<std::string, SymArrayMemoryObject *> symbolicArrayMap;
  std::map<std::string, TensorSizesMemoryObject *> tensorSizesMap;
  TensorIterator* inputIterator;
  std::map<std::string, std::pair<ref<Expr>, bool>> intArgAddBound; // whether add bound constraints of int arguments
  bool allIntArgAddBound = false;
  std::map<std::string, unsigned> intArrNames; // names of arrays whose content is int including constant array, bytes of value

  std::map<std::string, std::map<ref<Expr>, std::tuple<MemOp, MemoryObject::MemType, std::string>>> addressInLoop; // address: op:0(read),1(write),2(both), str: lineId
  std::map<std::string, std::vector<ref<Expr>>> dataRaces; // loop assemblyLine: [address] 
  std::map<ref<Expr>, ref<Expr>> io_expr_map; // integer overflow exprs 
...
public:
  ...
  void addArrConstraint(ref<Expr> e);
  ...
  void setCudaKernelConfig(const CUDAKernelConfig &config);
  const CUDAKernelConfig& getCudaKernelConfig() const;

  std::vector<ref<Expr>> getIOExprs() const {
    std::vector<ref<Expr>> keys;
    keys.reserve(io_expr_map.size());   // avoid reallocations
    for (const auto &kv : io_expr_map) {
