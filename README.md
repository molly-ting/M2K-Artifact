...
`<profile-out-dir>/out/Qwen_Qwen2-0.5B-Instruct.json` contains the profiling results generated using the default model configuration. The excerpt below shows the profiling information for the `rms_norm` kernel. The first two parameters are two-dimensional tensors. Their first dimension is the sequence length multiplied by the batch size, while their second dimension is the model-specific constant `896`. The third parameter is a one-dimensional tensor with a fixed size of `896`. The fourth parameter is the constant floating-point value `1e-06`.
