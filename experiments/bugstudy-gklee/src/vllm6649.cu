...
// FP16 type definition (replacement for cuda_fp16.h)
...
void test_overflow_bug() {
    ...
    cudaMalloc(&d_output, num_elements * sizeof(unsigned char));
    cudaMalloc(&d_scale, sizeof(float));
    
    cudaMemcpy(d_input, h_input, num_elements * sizeof(float), cudaMemcpyHostToDevice);
    
    float scale_val = 1.0f;
    cudaMemcpy(d_scale, &scale_val, sizeof(float), cudaMemcpyHostToDevice);
    ...
    cudaMemcpy(h_output_buggy, d_output, num_elements * sizeof(unsigned char), cudaMemcpyDeviceToHost);
    
    // Clear output for fixed kernel
    cudaMemset(d_output, 0, num_elements * sizeof(unsigned char));
    ...
    cudaMemcpy(h_output_fixed, d_output, num_elements * sizeof(unsigned char), cudaMemcpyDeviceToHost);
