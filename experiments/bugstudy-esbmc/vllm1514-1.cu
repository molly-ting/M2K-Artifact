...
int main() {
    ...
    // float* d_kv_cache = (float*)malloc(kv_cache_size * sizeof(float));
    // memset(d_kv_cache, 0, kv_cache_size * sizeof(float));
    float d_kv_cache[kv_cache_size];
    ...
    ESBMC_verify_kernel(paged_attention_kernel_buggy, block, grid, h_block_table, d_kv_cache, kv_block_stride);
