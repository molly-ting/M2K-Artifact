...
void run_test_case(const char* test_name, int k_global, int tp_size, int group_size, bool apply_fix) {
    ...
    CHECK_CUDA(hipMalloc(&d_input, num_elements * sizeof(float)));
    CHECK_CUDA(hipMalloc(&d_output, num_elements * sizeof(float)));

    if (num_scales > 0) {
        CHECK_CUDA(hipMalloc(&d_scales, num_scales * sizeof(float)));
        // Initialize valid scale
        float h_scale = 0.5f;
        CHECK_CUDA(hipMemcpy(d_scales, &h_scale, sizeof(float), hipMemcpyHostToDevice));
    }
