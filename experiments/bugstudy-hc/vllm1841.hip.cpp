...
int main() {
   // Configuration from user's specification
    ...
    CUDA_CHECK(hipMalloc(&d_positions, num_tokens * sizeof(int64_t)));
    CUDA_CHECK(hipMalloc(&d_query, num_tokens * num_heads * head_size * sizeof(float)));
    CUDA_CHECK(hipMalloc(&d_key, num_tokens * num_kv_heads * head_size * sizeof(float)));
    CUDA_CHECK(hipMalloc(&d_cos_sin_cache, actual_cache_size * sizeof(float)));
    
    // Copy to device
    CUDA_CHECK(hipMemcpy(d_positions, h_positions.data(), 
                         num_tokens * sizeof(int64_t), hipMemcpyHostToDevice));
    CUDA_CHECK(hipMemcpy(d_query, h_query.data(), 
                         num_tokens * num_heads * head_size * sizeof(float), hipMemcpyHostToDevice));
    CUDA_CHECK(hipMemcpy(d_key, h_key.data(), 
                         num_tokens * num_kv_heads * head_size * sizeof(float), hipMemcpyHostToDevice));
    CUDA_CHECK(hipMemcpy(d_cos_sin_cache, h_cos_sin_cache.data(), 
    ...
    // Cleanup
    CUDA_CHECK(hipFree(d_positions));
    CUDA_CHECK(hipFree(d_query));
    CUDA_CHECK(hipFree(d_key));
    CUDA_CHECK(hipFree(d_cos_sin_cache));
