...
int main() {
    ...
    hipMalloc(&d_positions, num_tokens * sizeof(int64_t));
    hipMalloc(&d_query, num_tokens * num_heads * head_size * sizeof(float));
    hipMalloc(&d_key, num_tokens * num_kv_heads * head_size * sizeof(float));
    hipMalloc(&d_cos_sin_cache, max_position * rot_dim * sizeof(float));

    // Copy to device
    hipMemcpy(d_positions, h_positions, num_tokens * sizeof(int64_t), hipMemcpyHostToDevice);
    hipMemcpy(d_query, h_query, num_tokens * num_heads * head_size * sizeof(float), hipMemcpyHostToDevice);
    hipMemcpy(d_key, h_key, num_tokens * num_kv_heads * head_size * sizeof(float), hipMemcpyHostToDevice);
    hipMemcpy(d_cos_sin_cache, h_cos_sin_cache, max_position * rot_dim * sizeof(float), hipMemcpyHostToDevice);
    ...
    // // Copy results back
    // hipMemcpy(h_query, d_query, num_tokens * num_heads * head_size * sizeof(float), hipMemcpyDeviceToHost);
    // hipMemcpy(h_key, d_key, num_tokens * num_kv_heads * head_size * sizeof(float), hipMemcpyDeviceToHost);

    // // Cleanup
    // hipFree(d_positions);
    // hipFree(d_query);
    // hipFree(d_key);
    // hipFree(d_cos_sin_cache);
