...
// CUDA FP16 definitions for older CUDA versions without hip/hip_fp16.h
...
__global__ void single_query_cached_kv_attention_kernel(
    ...
    out[threadIdx.x] = static_cast<T>(logits[token_idx]);
}
...
void single_query_cached_kv_attention_launcher(
    ...
    int logits_size = padded_max_context_len * sizeof(T);  // still the intentional bug

    // GKLEE will rewrite <<< >>> via fix_kernel; keep standard launch here.
    single_query_cached_kv_attention_kernel<T>
        <<<grid_size, BLOCK_SIZE, shared_mem_size, stream>>>(out, in, n);
}

int main() {
    typedef float T;   // no 'using' (C++11)
    ...
    single_query_cached_kv_attention_launcher<T>(d_out, d_in, n);
