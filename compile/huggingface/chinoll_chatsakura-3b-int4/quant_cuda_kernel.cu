...
void vecquant4matmul_cuda(
  torch::Tensor vec, // shape: [x0, x1]
  torch::Tensor mat, // shape: [infeatures // 8192, outfeatures]
  torch::Tensor mul, // shape: [x0, outfeatures]
  torch::Tensor scales, // shape: [1, outfeatures]
  torch::Tensor zeros, // shape: [1, outfeatures // 8192]
  int groupsize // infeatures
) {
  int batch = vec.size(0); // x0, size function returns int64_t, here cast to int, possibly overflow
  int vec_height = vec.size(1); // x1
  int height = mat.size(0); // infeatures//8192
  int width = mat.size(1); // outfeatures
  int zero_width = zeros.size(1); // outfeatures//8192

  dim3 blocks(
    (height + BLOCKHEIGHT4 - 1) / BLOCKHEIGHT4, // (infeatures//8192 + 31)/32
    (width + BLOCKWIDTH - 1) / BLOCKWIDTH, // (outfeatures + 255)/256
    batch // x0
  );
  dim3 threads(BLOCKWIDTH);// 256
...
template <typename scalar_t>
__global__ void VecQuant4MatMulKernel(
    const  scalar_t* __restrict__ vec, // shape: [x0, x1]
    const       int* __restrict__ mat, // shape: [infeatures // 8192, outfeatures]
           scalar_t* __restrict__ mul, // shape: [x0, outfeatures]
    const  scalar_t* __restrict__ scales, // shape: [1, outfeatures]
    const       int* __restrict__ zeros, // shape: [1, outfeatures // 8192]
    int batch, // x0
    int vec_height, // x1
    int height, // infeatures//8192
    int width, // outfeatures
    int zero_width, // outfeatures//8192
    int groupsize // infeatures
) {
  int b = blockIdx.z; // [0, x0-1]
  int h = BLOCKHEIGHT4 * blockIdx.x; // [0, 32*((infeatures//8192 + 31)/32 - 1)]
  int w = BLOCKWIDTH * blockIdx.y + threadIdx.x; // [0, 256*((outfeatures + 255)//256 - 1)+255]

  __shared__ scalar_t blockvec[BLOCKWIDTH];
  blockvec[threadIdx.x] = vec[b * vec_height + blockIdx.x * BLOCKWIDTH + threadIdx.x]; // [0, (x0-1)*x1 + ((infeatures//8192 + 31)/32 - 1)*256 + 255]
  __syncthreads();

  scalar_t res = 0;
  int i = width * h + w; // [0, outfeatures*32*((infeatures//8192 + 31)/32 - 1)+256*((outfeatures + 255)//256 - 1)+255]
  int g_h = h * 8; // [0, 8*32*((infeatures//8192 + 31)/32 - 1)]
  int k = 0;

  int z_w = w / 8; // [0, (256*((outfeatures + 255)//256 - 1)+255)/8]
  int z_mod = (w % 8) * 4; // [0, 28]
  ...
  while (k < BLOCKWIDTH) {
    ...
    int g = (g_h + k) / groupsize; // [0, (8*32*((infeatures//8192 + 31)/32 - 1)+255)/infeatures]
    scalar_t scale = scales[g * width + w]; // [0, ((8*32*((infeatures//8192 + 31)/32 - 1)+255)/infeatures)*outfeatures+256*((outfeatures + 255)//256 - 1)+255]
    scalar_t zero = scale * scalar_t(((as_unsigned(zeros[g * zero_width + z_w]) >> z_mod) & 0xF) + 1); // [0, ((8*32*((infeatures//8192 + 31)/32 - 1)+255)/infeatures) * (outfeatures//8192) + (256*((outfeatures + 255)//256 - 1)+255)/8]
